...
public class KafkaEventMapper {
  ...
  private com.jeannimi.messenger.adapter.kafka.event.ChatCreatedEvent toKafkaEvent(ChatCreatedEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.ChatCreatedEvent(
        event.chatId(), event.name(), event.type(), event.memberIds());
  }

  private com.jeannimi.messenger.adapter.kafka.event.ChatDeletedEvent toKafkaEvent(ChatDeletedEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.ChatDeletedEvent(event.chatId());
  }

  private com.jeannimi.messenger.adapter.kafka.event.ChatRenamedEvent toKafkaEvent(ChatRenamedEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.ChatRenamedEvent(
        event.chatId(), event.oldName(), event.newName());
  }

  private com.jeannimi.messenger.adapter.kafka.event.ChatMemberAddedEvent toKafkaEvent(
      ChatMemberAddedEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.ChatMemberAddedEvent(
        event.chatId(), event.userId(), event.username());
  }

  private com.jeannimi.messenger.adapter.kafka.event.ChatMemberRemovedEvent toKafkaEvent(
      ChatMemberRemovedEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.ChatMemberRemovedEvent(
        event.chatId(), event.userId());
  }

  private com.jeannimi.messenger.adapter.kafka.event.ChatMemberLeftEvent toKafkaEvent(
      ChatMemberLeftEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.ChatMemberLeftEvent(
        event.chatId(), event.userId());
  }

  private MessageSentEvent toKafkaEvent(
  ...
  private com.jeannimi.messenger.adapter.kafka.event.MessageReadEvent toKafkaEvent(MessageReadEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.MessageReadEvent(
        event.messageId(),
        event.chatId(),
        event.readerId(),
        event.readAt(),
  ...
  private com.jeannimi.messenger.adapter.kafka.event.MessageDeletedEvent toKafkaEvent(
      MessageDeletedEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.MessageDeletedEvent(
        event.messageId(), event.chatId(), event.deletedBy(), event.deletedAt());
  }

  private com.jeannimi.messenger.adapter.kafka.event.FileDeletionRequestedEvent toKafkaEvent(
      FileDeletionRequestedEvent event) {

    return new com.jeannimi.messenger.adapter.kafka.event.FileDeletionRequestedEvent(
